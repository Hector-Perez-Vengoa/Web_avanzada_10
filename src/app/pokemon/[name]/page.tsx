...
interface PokemonPageProps {
  // En App Router `params` puede ser una Promise que debe desempaquetarse con `await`.
...
export async function generateMetadata({ params }: Readonly<PokemonPageProps>): Promise<Metadata> {
  // Genera la metadata dinámica para cada página de Pokémon
  // `params` puede ser una Promise en el App Router, por eso lo await-eamos.
...
export default async function PokemonDetail({ params }: Readonly<PokemonPageProps>) {

  // `params` puede ser una Promise según Next.js App Router, desenvuélvelo antes de usar.
  const { name } = await params;
  const pokemon = await getPokemon(name);
...
// next: { revalidate: 86400 } permite realizar otra petición al servicio cada 24 horas. (ISR)
// generateStaticParams: genera los parámetros estáticos para los 151 registros. (SSG)
